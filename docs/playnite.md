...
MGS Master Hub replaces Playnite's six separate Master Collection entries with one launcher
...
| Action type | File |
| Path | the hub's installed exe, e.g. `C:\Program Files\MGS Master Hub\MGS Master Hub.exe` |
| Arguments | (none) |
...
|---|---|
| Action type | File |
| Path | `MGSMasterHub.exe` (same install path as above) |
| Arguments | `--game mgs3` |
