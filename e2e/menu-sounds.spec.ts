...
test("menu sounds follow semantic actions once, with no passive or ineffective-input sounds", async () => {
  ...
  for (const [index, sound] of MENU_SOUNDS.entries()) await writeFile(join(data, "sounds", `${sound}.wav`), silentWav((index + 5) / 100));
  await mkdir(join(data, "music", "mgs2"), { recursive: true });
  await writeFile(join(data, "music", "mgs2", "Custom Theme.wav"), silentWav(1));
  ...
  try {
    ...
    await page.addInitScript(() => {
      const events: { duration: number; time: number }[] = [];
      Object.assign(window, { soundEvents: events });
      const original = AudioBufferSourceNode.prototype.start;
      AudioBufferSourceNode.prototype.start = function (...args) {
        events.push({ duration: this.buffer?.duration ?? 0, time: Date.now() });
    ...
    async function expectSounds(expected: MenuSound[]) {
      ...
      const durations = await page.evaluate(() => (window as unknown as { soundEvents: { duration: number }[] }).soundEvents.splice(0).map(event => Math.round(event.duration * 100)));
      expect(durations).toEqual(expected.map(sound => MENU_SOUNDS.indexOf(sound) + 5));
    }
    await expectSounds([]);
