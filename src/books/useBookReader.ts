...
export function useBookReader(request: BookRequest) {
  ...
  const { gameId, kind, language, importedId } = request;
  useEffect(() => {
    let active = true;
    const pageCache = new PageWindow(async (index, signal) => {
      const result = await window.hub.getBookPage({ gameId, kind, language, importedId, page: index });
      signal.throwIfAborted();
    ...
    void window.hub.openBook({ gameId, kind, language, importedId }).then(result => {
      if (!active) return;
      if (!result.ok) throw new Error(result.error);
  ...
  }, [gameId, kind, language, importedId, attempt]);

  useEffect(() => {
    if (!document || document.gameId !== gameId || document.kind !== kind || document.language !== language || document.importedId !== importedId || !pages.current) return;
    let active = true;
    ...
    void pages.current.get(pageIndex).then(result => {
      ...
      saves.current = saves.current.then(async () => {
        if (!active) return;
        const saved = await window.hub.saveBookProgress({ gameId, kind, language, importedId, page: pageIndex });
    ...
    return () => { active = false; };
  }, [document, gameId, kind, language, importedId, pageIndex, pageAttempt]);
