...
export default function BonusContentScreen(props: BonusContentScreenProps) {
  ...
  const [screen, setScreen] = useState<"home" | "music" | "videos">("home");
  useEffect(() => {
    let active = true;
...
function BonusHome({ actionRef, lastInputKind, onClose, library, loading, error, retry, open, presentation }: BonusContentScreenProps & {
  ...
  return <main className="bonus-screen bonus-home" aria-label="Bonus Content" data-testid="bonus-content" data-art-volume={artVolume}>
    ...
    <div className="bonus-home-menu" aria-label="Bonus Content">
      {rows.map((row, index) => <button key={row.title} data-testid={index === 0 ? "bonus-back" : row.title === "Videos" ? "bonus-menu-videos" : row.title === "Digital Soundtrack" ? "bonus-menu-soundtrack" : "bonus-retry"} className={focus === index ? "bonus-selected" : ""}
      ...
      {!loading && !error && !library?.tracks.length && !library?.videos.length && <p>Install Bonus Content through Steam, then choose Retry.</p>}
